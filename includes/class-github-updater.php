...
class Github_Updater {
	...
	/**
	 * Plugin basename (e.g. "easy-logo-carousel/easy-logo-carousel.php").
	 *
	 * @var string
	 */
	...
	private function is_enabled(): bool {
		/**
		 * Filter whether GitHub auto-updates are enabled for Easy Logo Carousel.
		 *
		 ...
		 */
		return (bool) apply_filters( 'elc_updater_enabled', true );
	}
	...
	public function check_for_update( $transient ) {
		...
		if ( empty( $checked ) ) {
			...
			} elseif ( version_compare( ELC_VERSION, $release['version'], '>=' ) ) {
				$this->log( 'check_for_update: current version ' . ELC_VERSION . ' is up to date (latest: ' . $release['version'] . ').' );
			} else {
				$this->log( 'check_for_update: update available ' . ELC_VERSION . ' → ' . $release['version'] . '.' );
	...
	public function plugin_info( $result, $action, $args ) {
		...
		if ( is_array( $release ) ) {
			...
			$plugin_data = get_plugin_data( ELC_FILE, false, true );
	...
	public function clear_cache( $upgrader, $options ): void {
		if (
			...
			delete_transient( ELC_UPDATER_CACHE_KEY );
	...
	private function get_latest_release(): ?array {
		...
		$cached = get_transient( ELC_UPDATER_CACHE_KEY );

		if ( is_array( $cached ) ) {
			$this->log( 'get_latest_release: using cached release data.' );
			$release = $cached;
		} else {
			$url      = sprintf( 'https://api.github.com/repos/%s/releases/latest', ELC_UPDATER_GITHUB_REPO );
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( ! is_array( $body ) || empty( $body['tag_name'] ) ) {
					...
					if ( empty( $zip_url ) ) {
						...
						set_transient( ELC_UPDATER_CACHE_KEY, $release, ELC_UPDATER_CACHE_TTL );
					}
				}
			}
		}
	...
	private function log( string $message ): void {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( 'Easy Logo Carousel Github_Updater: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional debug logging.
		}
	}
	...
	private function log_error( string $message ): void {
		error_log( 'Easy Logo Carousel Github_Updater [error]: ' . $message ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Intentional error logging for updater failures.
	}
}
